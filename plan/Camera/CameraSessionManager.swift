...
final class CameraSessionManager: @unchecked Sendable {
    ...
    private let sessionQueue = DispatchQueue(label: "com.poonaapp.camera.session")
    ...
    func setupSession() async {
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            sessionQueue.async { [weak self] in
                ...
                // Create a fresh session each time — reused sessions after teardown are unreliable
    ...
    func teardown() {
        sessionQueue.async { [weak self] in
            ...
            // Clear delegate before removing output to prevent frame delivery to dead session
